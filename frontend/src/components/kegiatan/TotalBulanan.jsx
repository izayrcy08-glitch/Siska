...
import { FileText, AlertCircle, Settings, AlertTriangle } from 'lucide-react';
import { BULAN_INDONESIA } from '../../utils/timeUtils';
import { getMissingPegawaiFields } from '../../utils/storage';
...
const TotalBulanan = ({
  ...
  const missing = getMissingPegawaiFields(settings);
    ...
    <div className="px-4 pb-6 mt-2">
      <div className="bg-gradient-to-r from-blue-900 to-blue-800 rounded-2xl p-4 text-white mb-4 shadow-md">
        ...
        <p className="text-blue-200 text-sm mt-0.5">
          {namaBulan} {tahun}
          {savedDaysCount > 0 ? ` · ${savedDaysCount} hari tersimpan` : ''}
        </p>
      </div>
      ...
      <button
        ...
        className={`w-full h-14 rounded-xl flex items-center justify-center gap-2 text-base font-semibold transition-all ${
          isPDFReady
            ? 'bg-teal-700 text-white hover:bg-teal-800 active:scale-[0.99] shadow-sm'
      ...
      {!isPDFReady && (
        <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3">
          ...
          <p className="text-xs text-amber-700 mb-2 ml-5">
            Lengkapi di Pengaturan: {missing.join(', ') || 'Nama, NIP, Jabatan'}.
          </p>
          {onGoToSettings && (
            <button
              type="button"
      ...
      {isPDFReady && draftCount > 0 && (
        <div className="mt-3 bg-orange-50 border border-orange-200 rounded-xl p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-orange-800">
            Ada <span className="font-semibold">{draftCount} hari</span> dengan isian yang belum ditekan{' '}
            <span className="font-semibold">Simpan</span>. Hari tersebut tidak masuk total PDF sampai disimpan.
          </p>
        </div>
      )}
    </div>
